...
IndexArrayID indexID[MAX_STUDENTS];
...
void sortByID(IndexArrayID* arr, int size) {
	for (int i = 1; i < size; i++) {
		IndexArrayID key = arr[i];
		int j = i - 1;
...
int searchById(int left, int right, int key) {
	if (left > right) return -1;
	int mid = left + (right - left) / 2;
	if (indexID[mid].id == key) return indexID[mid].recNum;
	if (indexID[mid].id < key) searchById(mid + 1, right, key);
	else searchById(left, mid - 1, key);
}

int searchByName(const string& key) {
	string normTarget = normalize(key);
...
int main() {
	...
	while (!exit) {
		...
		case 4:
			cout << "Введите ID: ";
			cin >> id;
			key = searchById(0, studentsNumber - 1, id);
			if (key != -1) {
				students[key].print();
